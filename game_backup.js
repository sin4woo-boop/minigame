...
const TEST_EVOLUTION_KEYS = ["pea+spore", "corn+spike", "cherry+spike", "spore+vine", "pea+vine"];
...
const EVOLUTIONS = {
  "pea+spore": {
    title: "진화: 독침 연쇄",
    desc: "완두콩+버섯 합체. 바운스할 때마다 독 구름을 남기며 5회 튕깁니다.",
    icon: "spore",
    keep: "pea",
    consume: "spore",
    apply: () => {
      const keep = state.player.weapons.pea;
      const consume = state.player.weapons.spore;
      ...
      keep.damage += 6;
      keep.bounce = 5;
      keep.bounceSpore = true;
      keep.level = MAX_WEAPON_LEVEL;
    },
  },
  "corn+spike": {
    title: "진화: 가시 폭풍",
    desc: "옥수수+선인장 합체. 관통 산탄 10발을 폭풍처럼 쏟아냅니다.",
    icon: "corn",
    keep: "corn",
    consume: "spike",
    apply: () => {
  ...
  "cherry+spike": {
    title: "진화: 핵폭탄",
    desc: "체리+선인장 합체. 폭발 시 관통 가시 파편 12발이 날아갑니다.",
    icon: "cherry",
    keep: "cherry",
    consume: "spike",
    apply: () => {
      ...
      consume.lockedByEvolution = true;
      keep.label = "핵폭탄";
      keep.evolvedKey = "cherry+spike";
      keep.damage += 30;
      keep.splash = Math.round(keep.splash * 1.5);
      keep.shrapnel = 12;
      keep.level = MAX_WEAPON_LEVEL;
    },
  },
  "spore+vine": {
    title: "진화: 맹독 정원",
    desc: "버섯+덩굴 합체. 포자 구름이 적을 포박하고 독 데미지 2배.",
    icon: "spore",
    keep: "spore",
    consume: "vine",
    apply: () => {
      ...
      keep.damage = Math.round(keep.damage * 2);
      keep.splash += 25;
      keep.cloudDuration = (keep.cloudDuration || 3.0) + 1.5;
      keep.cloudSnare = true;
      keep.level = MAX_WEAPON_LEVEL;
    },
  },
  "pea+vine": {
    title: "진화: 생명 사슬",
    desc: "완두콩+덩굴 합체. 바운스 시 적을 덩굴 사슬로 연결, 이속 50% 감소+지속 데미지.",
    icon: "vine",
    keep: "pea",
    consume: "vine",
    apply: () => {
      ...
      consume.lockedByEvolution = true;
      keep.label = "생명 사슬";
      keep.evolvedKey = "pea+vine";
      keep.damage += 5;
      keep.bounce = Math.max(keep.bounce || 3, 4);
      keep.vineChain = true;
      keep.level = MAX_WEAPON_LEVEL;
    },
  },
...
function createWeaponConfig() {
  ...
  for (const [id, cfg] of Object.entries(BALANCE.weapons)) {
    ...
    if (id === "pea") {
      weapons[id].bounce = cfg.bounce || 3;
      weapons[id].bounceRange = cfg.bounceRange || 140;
      weapons[id].bounceDmgMul = cfg.bounceDmgMul || 1.1;
    }
    if (id === "spike") {
      weapons[id].pierceFalloff = cfg.pierceFalloff ?? 0;
    }
    if (id === "spore") {
      weapons[id].cloudDuration = cfg.cloudDuration || 3.0;
      weapons[id].tickRate = cfg.tickRate || 0.5;
      weapons[id].slowMul = cfg.slowMul || 0.35;
    }
    if (id === "corn") {
      weapons[id].pellets = cfg.pellets || 7;
      weapons[id].spreadAngle = cfg.spreadAngle || 0.7;
    }
    if (id === "cherry") {
      weapons[id].fuseTime = cfg.fuseTime || 1.8;
      weapons[id].knockback = cfg.knockback || 80;
    }
    if (id === "vine") {
      weapons[id].snareDuration = cfg.snareDuration || 2.0;
      weapons[id].pullRange = cfg.pullRange || 90;
      weapons[id].pullForce = cfg.pullForce || 120;
    }
  }
  return weapons;
}
...
function resetGame() {
  ...
  state.bossDirector = { nextSpawnTime: BALANCE.wave.boss.spawnSecs[0] || 60, spawned: 0, lastVariant: null };
...
function setAllWeaponsBaseState() {
  for (const [id, w] of Object.entries(state.player.weapons)) {
    ...
    w.evolvedKey = "";
    // Reset weapon-specific
    if (id === "pea") {
      w.bounce = base.bounce || 3;
      w.bounceRange = base.bounceRange || 140;
      w.bounceDmgMul = base.bounceDmgMul || 1.1;
      w.bounceSpore = false;
      w.vineChain = false;
    }
    if (id === "spike") { w.pierceFalloff = base.pierceFalloff ?? 0; }
    if (id === "spore") {
      w.cloudDuration = base.cloudDuration || 3.0;
      w.tickRate = base.tickRate || 0.5;
      w.slowMul = base.slowMul || 0.35;
      w.cloudSnare = false;
    }
    if (id === "corn") {
      w.pellets = base.pellets || 7;
      w.spreadAngle = base.spreadAngle || 0.7;
      w.pelletPierce = 0;
    }
    if (id === "cherry") {
      w.fuseTime = base.fuseTime || 1.8;
      w.knockback = base.knockback || 80;
      w.shrapnel = 0;
    }
    if (id === "vine") {
      w.snareDuration = base.snareDuration || 2.0;
      w.pullRange = base.pullRange || 90;
      w.pullForce = base.pullForce || 120;
    }
  }
}
...
function spawnBossZombie(variant = "hammer") {
  const side = Math.floor(rand(0, 4));
  let x = 0;
  let y = 0;
  ...
  const c = BALANCE.enemies.boss[variant] || BALANCE.enemies.boss.hammer;
  const preset = getPreset();
  ...
  if (variant === "hammer") {
    z.slamTimer = c.slam.cooldown;
    z.chargeTimer = c.charge.cooldown;
    z.eqTimer = c.earthquake.cooldown;
  } else if (variant === "spore") {
    z.blastTimer = c.toxicBlast.cooldown;
    z.splitTimer = c.sporeSplit.cooldown;
    z.breathTimer = c.decayBreath.cooldown;
  } else if (variant === "morningStar") {
    z.maceTimer = c.maceSwing.cooldown;
    z.chainTimer = c.chainThrow.cooldown;
    z.berserkActive = false;
  }
...
function updateBossDirector() {
  if (state.testMode) return;
  const spawnSecs = BALANCE.wave.boss.spawnSecs || [60, 180, 300];
  const bossVariants = ["hammer", "spore", "morningStar"];

  if (state.bossDirector.spawned >= spawnSecs.length) return; // All bosses spawned
  if (state.time < state.bossDirector.nextSpawnTime) return;
  if (hasActiveBoss()) return;

  const currentVariant = bossVariants[state.bossDirector.spawned] || "hammer";
  spawnBossZombie(currentVariant);
  state.bossStats.encounters += 1;
  state.bossDirector.spawned += 1;

  if (state.bossDirector.spawned < spawnSecs.length) {
    state.bossDirector.nextSpawnTime = spawnSecs[state.bossDirector.spawned];
  } else {
    state.bossDirector.nextSpawnTime = Infinity;
  }

  let bossNameInfo = "망치 대장";
  if (currentVariant === "spore") bossNameInfo = "포자 군주";
  if (currentVariant === "morningStar") bossNameInfo = "모닝스타 집행자";
  ...
  state.effects.push({
    ...
    maxLife: 1.35,
    size: 1,
    text: `보스 출현! ${bossNameInfo}`,
    subText: "고유한 패턴을 피하면서 처치하세요!",
  });
...
function shootWeapon(weapon, target, weaponId = null) {
  ...
  if (weaponId === "pea") {
    state.bullets.push({
      ...
      hitIds: new Set(),
      bounce: weapon.bounce || 3,
      bounceRange: weapon.bounceRange || 140,
      bounceDmgMul: weapon.bounceDmgMul || 1.1,
      bounceSpore: weapon.bounceSpore || false,
      vineChain: weapon.vineChain || false,
      lastHit: null,
    });
    return;
  }

  // === SPIKE: Piercing projectile ===
  if (weaponId === "spike") {
    state.bullets.push({
      kind: "spikePierce",
      weaponId,
      ...
      rotation: Math.atan2(dy, dx),
      hitIds: new Set(),
      pierce: weapon.pierce || 5,
      pierceFalloff: weapon.pierceFalloff ?? 0,
      trailPoints: [],
    });
    return;
  }

  // === SPORE: Poison cloud placement ===
  if (weaponId === "spore") {
    const cloudSnare = weapon.cloudSnare || false;
    state.bullets.push({
      kind: "sporeZone",
      weaponId,
      ...
      damage: weapon.damage,
      life: weapon.cloudDuration || 3.0,
      maxLife: weapon.cloudDuration || 3.0,
      tickRate: weapon.tickRate || 0.5,
      tickTimer: 0,
      slowMul: weapon.slowMul || 0.35,
      cloudSnare,
    });
    return;
  }

  // === CORN: Shotgun pellets ===
  if (weaponId === "corn") {
    const baseAngle = Math.atan2(dy, dx);
    const pelletCount = weapon.pellets || 7;
    const halfSpread = (weapon.spreadAngle || 0.7) / 2;
    const pelletPierce = weapon.pelletPierce || 0;
    for (let i = 0; i < pelletCount; i++) {
      const t = pelletCount > 1 ? i / (pelletCount - 1) : 0.5;
      const ang = baseAngle - halfSpread + t * (weapon.spreadAngle || 0.7) + (Math.random() - 0.5) * 0.06;
      const spd = weapon.speed * (0.9 + Math.random() * 0.2);
      state.bullets.push({
        kind: "cornPellet",
        weaponId,
        x: p.x, y: p.y,
  ...
  if (weaponId === "cherry") {
    state.bullets.push({
      ...
      sprite: weapon.sprite,
      rotation: Math.atan2(dy, dx),
      fuseTime: weapon.fuseTime || 1.8,
      fuseTimer: 0,
      stuck: false,
      stuckTarget: null,
      knockback: weapon.knockback || 80,
      shrapnel: weapon.shrapnel || 0,
    });
    return;
  }
  ...
  if (weaponId === "vine") {
    state.bullets.push({
      ...
      splash: weapon.splash,
      damage: weapon.damage,
      life: weapon.snareDuration + 1.0,
      maxLife: weapon.snareDuration + 1.0,
      warmup: 0.35,
      triggered: false,
      snareDuration: weapon.snareDuration || 2.0,
      pullRange: weapon.pullRange || 90,
      pullForce: weapon.pullForce || 120,
      snaredZombies: new Set(),
    });
    return;
  }
}
...
function getWeaponEvolutionMode(weaponId, weapon = null) {
  ...
  if (weaponId === "pea" && evolvedKey === "pea+spore") return "poisonBounce";
  if (weaponId === "pea" && evolvedKey === "pea+vine") return "vineChain";
  if (weaponId === "corn" && evolvedKey === "corn+spike") return "thornStorm";
  if (weaponId === "cherry" && evolvedKey === "cherry+spike") return "nuclearBomb";
...
function updateZombies(dt) {
  ...
  for (let i = state.zombies.length - 1; i >= 0; i -= 1) {
    ...
    if (z.type === "boss") {
      const cfg = bossCfg[z.variant];
      z.x += (dx / len) * z.speed * dt * finalMoveMul;
      z.y += (dy / len) * z.speed * dt * finalMoveMul;

      z.summonTimer -= dt;
      if (z.summonTimer <= 0) {
        for (let s = 0; s < cfg.summonCount; s += 1) spawnBasicNear(z.x, z.y);
        z.summonTimer = cfg.summonCooldown;
        state.effects.push({ type: "pop", x: z.x, y: z.y, life: 0.35, maxLife: 0.35, size: 46 });
      }

      if (z.variant === "hammer") {
        if (z.currentAction === "idle") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.slamTimer -= dt;
            z.chargeTimer -= dt;
            ...
            if (z.eqTimer <= 0) {
              z.currentAction = "earthquake";
              z.actionTimer = cfg.earthquake.windup;
              z.eqCount = cfg.earthquake.count;
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.earthquake.windup, maxLife: cfg.earthquake.windup, size: cfg.earthquake.radius, warnRgb: "255, 100, 100" });
            } else if (z.chargeTimer <= 0) {
              z.currentAction = "chargeWindup";
              z.actionTimer = cfg.charge.windup;
              z.chargeDx = dx / len;
              z.chargeDy = dy / len;
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.charge.windup, maxLife: cfg.charge.windup, size: 40, dash: [4, 4] });
            } else if (z.slamTimer <= 0 && len <= cfg.slam.radius + 60) {
              z.currentAction = "slamWindup";
              z.actionTimer = cfg.slam.windup;
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.slam.windup, maxLife: cfg.slam.windup, size: cfg.slam.radius });
            }
          }
        } else if (z.currentAction === "earthquake") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            applySplashDamage(z.x, z.y, cfg.earthquake.radius, cfg.earthquake.damage, z);
            bumpScreenShake(12);
            if (len <= cfg.earthquake.radius + p.r) applyPlayerDamage(cfg.earthquake.damage, z.x, z.y);

            z.eqCount -= 1;
            if (z.eqCount > 0) {
              z.actionTimer = cfg.earthquake.delay;
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.earthquake.delay, maxLife: cfg.earthquake.delay, size: cfg.earthquake.radius, warnRgb: "255, 100, 100" });
            } else {
              z.eqTimer = cfg.earthquake.cooldown;
              z.currentAction = "idle";
              z.actionTimer = 1.0;
            }
          }
        } else if (z.currentAction === "chargeWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.currentAction = "charging";
            z.actionTimer = cfg.charge.duration;
            bumpScreenShake(8);
          }
        } else if (z.currentAction === "charging") {
          z.actionTimer -= dt;
          z.x += z.chargeDx * cfg.charge.speed * dt;
          z.y += z.chargeDy * cfg.charge.speed * dt;
          if (len <= p.r + z.r + 10) applyPlayerDamage(cfg.charge.damage * dt * 4, z.x, z.y);
          if (z.actionTimer <= 0) {
            z.chargeTimer = cfg.charge.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 0.5;
          }
        } else if (z.currentAction === "slamWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            applySplashDamage(z.x, z.y, cfg.slam.radius, cfg.slam.damage, z);
            bumpScreenShake(10);
            if (len <= cfg.slam.radius + p.r) applyPlayerDamage(cfg.slam.damage, z.x, z.y);
            z.slamTimer = cfg.slam.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 0.8;
          }
        }
      } else if (z.variant === "spore") {
        if (z.currentAction === "idle") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.blastTimer -= dt;
            ...
            if (z.blastTimer <= 0) {
              z.currentAction = "blastWindup";
              z.actionTimer = cfg.toxicBlast.windup;
              z.blastTargetX = p.x;
              z.blastTargetY = p.y;
              state.effects.push({ type: "warn", x: p.x, y: p.y, life: cfg.toxicBlast.windup, maxLife: cfg.toxicBlast.windup, size: cfg.toxicBlast.radius, warnRgb: "180, 255, 100" });
            } else if (z.splitTimer <= 0) {
              z.currentAction = "splitWindup";
              z.actionTimer = cfg.sporeSplit.windup;
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.sporeSplit.windup, maxLife: cfg.sporeSplit.windup, size: 60, warnRgb: "200, 100, 255", dash: [5, 5] });
            } else if (z.breathTimer <= 0 && len <= cfg.decayBreath.range) {
              z.currentAction = "breathWindup";
              z.actionTimer = cfg.decayBreath.windup;
            }
          }
        } else if (z.currentAction === "blastWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            state.hazards.push({
              type: "casterBlast",
              x: z.blastTargetX,
              y: z.blastTargetY,
              r: cfg.toxicBlast.radius,
              life: cfg.toxicBlast.duration,
              armTime: 0.1,
              activeTime: 0.2,
              damage: cfg.toxicBlast.damage,
              fired: true,
            });
            z.blastTimer = cfg.toxicBlast.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 1.0;
          }
        } else if (z.currentAction === "splitWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            for (let i = 0; i < cfg.sporeSplit.count; i++) {
              const ang = (Math.PI * 2 * i) / cfg.sporeSplit.count;
              state.bullets.push({
                kind: "bullet",
                weaponId: "zombieSpore",
                x: z.x,
                y: z.y,
                vx: Math.cos(ang) * cfg.sporeSplit.speed,
                vy: Math.sin(ang) * cfg.sporeSplit.speed,
                r: cfg.sporeSplit.radius,
                damage: cfg.sporeSplit.damage,
                life: 3.5,
                sprite: "spore",
                splash: 0,
                pierce: 0,
            ...
            z.splitTimer = cfg.sporeSplit.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 1.0;
          }
        } else if (z.currentAction === "breathWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.currentAction = "breathing";
            z.actionTimer = cfg.decayBreath.duration;
            z.breathAng = Math.atan2(dy, dx);
          }
        } else if (z.currentAction === "breathing") {
          z.actionTimer -= dt;
          z.breathAng = Math.atan2(dy, dx); // Track player
          const pAng = Math.atan2(p.y - z.y, p.x - z.x);
          let adiff = Math.abs(pAng - z.breathAng);
          if (adiff > Math.PI) adiff = Math.PI * 2 - adiff;

          if (len <= cfg.decayBreath.range && adiff <= cfg.decayBreath.angle / 2) {
            applyPlayerDamage(cfg.decayBreath.damagePerSec * dt, z.x, z.y);
            p.moveMag = Math.max(0, p.moveMag - dt * 2); // Slow effect
          }
          if (Math.random() < 0.2) {
            const spread = (Math.random() - 0.5) * cfg.decayBreath.angle;
            state.effects.push({ type: "poisonCloud", x: z.x + Math.cos(z.breathAng + spread) * 40, y: z.y + Math.sin(z.breathAng + spread) * 40, vx: Math.cos(z.breathAng + spread) * 120, vy: Math.sin(z.breathAng + spread) * 120, life: 0.6, maxLife: 0.6, size: 25 });
          }
          if (z.actionTimer <= 0) {
            z.breathTimer = cfg.decayBreath.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 0.5;
          }
        }
      } else if (z.variant === "morningStar") {
        if (!z.berserkActive) {
          const hpRatio = z.hp / z.maxHp;
          if (hpRatio <= cfg.berserk.threshold) {
            z.berserkActive = true;
            z.speed *= cfg.berserk.speedMul;
            z.currentAction = "berserkCry";
            z.actionTimer = 1.5;
            state.effects.push({ type: "banner", x: canvas.width / 2, y: 66, life: 1.5, maxLife: 1.5, size: 1, text: "집행자 폭주!", subText: "방어력 40% 증가 & 불타는 흔적 생성" });
            bumpScreenShake(15);
          }
        } else {
          if (Math.random() < 0.15 * dt * 60) {
            applySplashDamage(z.x, z.y, cfg.berserk.burnRadius, cfg.berserk.burnDamage, z);
          }
        }
        ...
        if (z.currentAction === "idle") {
          ...
          if (z.actionTimer <= 0) {
            z.maceTimer -= dt;
            z.chainTimer -= dt;
            if (z.chainTimer <= 0 && len > 180) {
              z.currentAction = "chainWindup";
              z.actionTimer = cfg.chainThrow.windup;
              z.chainAng = Math.atan2(dy, dx);
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.chainThrow.windup, maxLife: cfg.chainThrow.windup, size: cfg.chainThrow.range, dash: [10, 10], warnRgb: "150, 150, 150" });
            } else if (z.maceTimer <= 0 && len <= cfg.maceSwing.range) {
              z.currentAction = "maceWindup";
              z.actionTimer = cfg.maceSwing.windup;
              z.maceDir = dx >= 0 ? 1 : -1;
              state.effects.push({ type: "warn", x: z.x, y: z.y, life: cfg.maceSwing.windup, maxLife: cfg.maceSwing.windup, size: z.r * 2.05, warnRgb: "255, 178, 116", dash: [6, 6] });
            }
          }
        } else if (z.currentAction === "berserkCry") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.currentAction = "idle";
            z.actionTimer = 0.5;
          }
        } else if (z.currentAction === "chainWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.currentAction = "chainThrowing";
            z.actionTimer = cfg.chainThrow.range / cfg.chainThrow.speed;
            z.chainDist = 0;
            playSfxEvent("playerHit"); // generic heavy swoosh
          }
        } else if (z.currentAction === "chainThrowing") {
          z.actionTimer -= dt;
          z.chainDist += cfg.chainThrow.speed * dt;
          const hx = z.x + Math.cos(z.chainAng) * z.chainDist;
          const hy = z.y + Math.sin(z.chainAng) * z.chainDist;
          const hdx = p.x - hx;
          const hdy = p.y - hy;
          if (hdx * hdx + hdy * hdy <= cfg.chainThrow.width * cfg.chainThrow.width) {
            applyPlayerDamage(cfg.chainThrow.damage, hx, hy);
            p.x = z.x + Math.cos(z.chainAng) * 60; // Pull
            p.y = z.y + Math.sin(z.chainAng) * 60;
            z.actionTimer = 0; // End early on hit
          }
          if (z.actionTimer <= 0) {
            z.chainTimer = cfg.chainThrow.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 0.8;
          }
        } else if (z.currentAction === "maceWindup") {
          z.actionTimer -= dt;
          if (z.actionTimer <= 0) {
            z.currentAction = "maceSwinging";
            z.actionTimer = cfg.maceSwing.swingTime;
            z.maceActiveMax = cfg.maceSwing.swingTime;
            z.maceAngle = -1.15 * z.maceDir;
            z.maceHitLock = 0;
            bumpScreenShake(7.5);
          }
        } else if (z.currentAction === "maceSwinging") {
          z.actionTimer -= dt;
          const swingT = clamp(1 - (z.actionTimer / Math.max(0.01, z.maceActiveMax)), 0, 1);
          z.maceAngle = (-1.15 + swingT * 2.3) * z.maceDir;
          const chainLen = z.r * cfg.maceSwing.lengthMul;
          const headX = z.x + Math.cos(z.maceAngle) * chainLen;
          const headY = z.y - z.r * 0.16 + Math.sin(z.maceAngle) * chainLen * 0.62;
          z.maceHitLock = Math.max(0, z.maceHitLock - dt);
          if (z.maceHitLock <= 0) {
            const mdx = p.x - headX;
            const mdy = p.y - headY;
            const hitR = cfg.maceSwing.headRadius + p.r;
            if (mdx * mdx + mdy * mdy <= hitR * hitR) {
              applyPlayerDamage(cfg.maceSwing.damage, headX, headY);
              z.maceHitLock = 0.22;
              bumpScreenShake(8.2);
            }
          }
          if (z.actionTimer <= 0) {
            z.maceTimer = cfg.maceSwing.cooldown;
            z.currentAction = "idle";
            z.actionTimer = 0.5;
          }
        }
...
function hitZombie(zombie, damage, sourceWeaponId = null, hitX = null, hitY = null) {
  ...
  // Set default hit coordinates to the zombie's current position if not provided
...
function updateEffects(dt) {
  for (let i = state.effects.length - 1; i >= 0; i -= 1) {
    ...
    if (ef.type === "swampBubble") {
      ef.x += (ef.vx || 0) * dt;
      ef.y -= (ef.vy || 10) * dt;
      // Wobble
      ef.x += Math.sin(state.time * 5 + ef.y * 0.1) * 0.5;
    }
...
function drawBackground() {
  ...
  // White road markings (Crosswalks)
  ctx.fillStyle = "rgba(240, 240, 240, 0.85)";
  const cwX = canvas.width - 120;
  for (let y = 80; y < canvas.height - 80; y += 32) {
    ctx.fillRect(cwX, y, 80, 14);
  }

  // Dashed lane lines
  const laneY = 240;
  for (let x = 0; x < canvas.width - 160; x += 80) {
    ctx.fillRect(x, laneY, 40, 12);
  }
}
...
function drawSceneAtmosphere() {
  ...
  for (let i = 0; i < 4; i += 1) {
    const period = 18 + i * 5;
    const offset = i * 120;
    const x = ((t * period + offset) % (canvas.width + 400)) - 200;
    const y = 30 + i * 40;
    const angle = -0.25 + Math.sin(t * 0.2 + i) * 0.05; // Gently swaying rays

    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
    const rayLen = canvas.height * 1.2;
    const beam = ctx.createLinearGradient(0, 0, rayLen, 0);
    const alpha = 0.04 + Math.sin(t * 0.8 + i) * 0.02;
    beam.addColorStop(0, "rgba(255, 250, 220, 0)");
    beam.addColorStop(0.3, `rgba(255, 250, 220, ${alpha})`);
    beam.addColorStop(0.7, `rgba(255, 250, 220, ${alpha * 0.5})`);
...
function draw() {
  ...
  for (const ef of state.effects) {
    ...
    // Custom leaf burst effect
    if (ef.type === "leafBurst") {
      ctx.save();
      ctx.translate(ef.x, ef.y);
      ctx.rotate(ef.rot + ef.life * 5); // spinning leaf
      ctx.fillStyle = `rgba(100, 220, 100, ${alpha})`;
      ctx.beginPath();
      ctx.ellipse(0, 0, ef.size, ef.size * 0.5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
      // Move effect
      ef.x += (ef.vx || 0) * (1 / 60); // approx dt
      ef.y += (ef.vy || 0) * (1 / 60);
      ef.vx *= 0.95;
      ef.vy *= 0.95;
    }

    // Custom thorn hit effect
    if (ef.type === "thornHit") {
      ctx.strokeStyle = `rgba(255, 204, 68, ${alpha})`;
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      const endX = ef.x + Math.cos(ef.rot) * ef.size;
      const endY = ef.y + Math.sin(ef.rot) * ef.size;
    ...
    // Custom swamp bubble effect
    if (ef.type === "swampBubble") {
      ctx.fillStyle = `rgba(170, 220, 160, ${alpha * 0.6})`;
      ctx.beginPath();
      ctx.arc(ef.x, ef.y, ef.size, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = `rgba(200, 255, 180, ${alpha * 0.8})`;
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.arc(ef.x, ef.y, ef.size, 0, Math.PI * 2);
      ctx.stroke();
    ...
    // Custom popcorn burst effect
    if (ef.type === "popcornBurst") {
      ctx.save();
      ctx.translate(ef.x, ef.y);
      ctx.rotate(ef.rot + ef.life * 8); // spinning popcorn

      // Buttery yellow
      ctx.fillStyle = `rgba(255, 230, 120, ${alpha})`;
      ctx.beginPath();
      ctx.arc(0, -ef.size * 0.3, ef.size * 0.6, 0, Math.PI * 2);
      ctx.arc(-ef.size * 0.4, ef.size * 0.3, ef.size * 0.5, 0, Math.PI * 2);
      ctx.arc(ef.size * 0.4, ef.size * 0.3, ef.size * 0.5, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(0, 0, ef.size * 0.7, 0, Math.PI * 2);
      ctx.fill();
    ...
    // Custom cherry burst effect
    if (ef.type === "cherryBurst") {
      const r = ef.size * (0.4 + (1 - alpha) * 0.6);
      // Flashing core
      ctx.fillStyle = (Math.floor(state.time * 30) % 2 === 0)
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(ef.x, ef.y, r * 1.5, 0, Math.PI * 2);
      ctx.stroke();
    }

    // Custom vine burst effect
    if (ef.type === "vineBurst") {
      ctx.save();
      ctx.translate(ef.x, ef.y);
      const r = ef.size * (0.3 + (1 - alpha) * 0.7);

      // Dirt particles
      ctx.fillStyle = `rgba(100, 70, 40, ${alpha})`;
      for (let d = 0; d < 6; d++) {
        const a = (Math.PI * 2 * d) / 6 + ef.life;
        ctx.beginPath();
        ctx.arc(Math.cos(a) * r * 0.6, Math.sin(a) * r * 0.6, r * 0.15, 0, Math.PI * 2);
        ctx.fill();
      }

      // Thorny vines erupting
      ctx.strokeStyle = `rgba(40, 150, 50, ${alpha})`;
      ctx.lineWidth = 4;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      for (let k = 0; k < 5; k += 1) {
        ...
        ctx.lineTo(Math.cos(a + 0.1) * r * 0.7, Math.sin(a + 0.1) * r * 0.7);
  ...
  for (const z of state.zombies) {
    ...
    if (images.zombie) {
      ...
      if (z.type === "hammer" && images.hammerZombie) {
        ...
        ctx.stroke();
      } else if (z.type === "boss" && (images.bossZombieCaster || images.bossZombieCrusher)) {
        const variant = z.variant || "hammer";
        const baseScale = variant === "morningStar" ? 3.28 : variant === "spore" ? 3.18 : 3.08;
        const bossScale = variant === "spore" ? baseScale * 0.92 : baseScale * 1.06;
        const bossImg = variant === "spore" ? images.bossZombieCaster : images.bossZombieCrusher;
        ...
        if (variant === "spore") {
          ...
          const maceCfg = BALANCE.enemies.boss.morningStar.maceSwing;
          const angle = z.currentAction === "maceSwinging"
            ? (z.maceAngle || 0)
...
function updateHud() {
  ...
  if (state.testMode) {
    ...
    bossHudEl.classList.remove("hidden");
    let name = "망치 대장";
    if (boss.variant === "spore") name = "포자 군주";
    if (boss.variant === "morningStar") name = "모닝스타 집행자";
